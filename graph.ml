...
let create ((): unit) : t = Array.make_matrix nb_syms nb_syms Positions.empty

let add_edge (g: t) (s1: sym) (s2: sym) (i: int) : unit =
  let i1 = to_int s1 in let i2 = to_int s2 in
...
let fold_over_connected (g: t) (f: 'a -> sym -> 'a) (x: 'a) (s: sym) : 'a =
  ...
  let filtered = List.filter_map (fun o -> match o with | None -> None | Some i -> Some (of_int i)) neigh in
...
let rec print_path (g: t) (o: Pervasives.out_channel) (l: sym list) : unit = match l with
  ...
  | h1 :: h2 :: t -> let i1 = to_int h1 in let i2 = to_int h2 in
    Printf.fprintf o "%c -{ " (to_char h1);
    print_set g.(i1).(i2) o;
    Printf.fprintf o "}-> ";
    print_path g o (h2 :: t)
